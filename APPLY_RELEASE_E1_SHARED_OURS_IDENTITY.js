...
const RELEASE = 'E1 — Shared/Ours Visual Identity';
...
let nextBadge;
try {
  nextCard = patchCard(read(cardPath));
  nextBadge = patchIdentityBadge(read(badgePath));
} catch (error) {
  fail(error instanceof Error ? error.message : String(error));
}

fs.writeFileSync(path.join(root, cardPath), nextCard, 'utf8');
fs.writeFileSync(path.join(root, badgePath), nextBadge, 'utf8');

console.log('[E1] Shared/Ours identity patches applied (or already present).');
audit();
