...
export default function CalendarScreen() {
  const theme = useAppTheme(); const params = useLocalSearchParams<{ focus?: string; prefillTitle?: string; prefillDescription?: string; prefillDuration?: string; sourceActivityId?: string }>(); const { colorForUser, profile, partnerProfile } = useWorkspace();
  ...
  const renderOccurrence = (occurrence: EventOccurrence) => { const event = occurrence.event; return <Card key={occurrence.key} participantColor={colorForUser(event.creator_id)} style={{ gap: theme.spacing.sm }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}><Pressable accessibilityRole="button" onPress={() => beginEdit(event)} style={{ flex: 1, gap: 5 }}><AppText variant="caption" tone="secondary">{occurrenceTime(occurrence).toUpperCase()}</AppText><AppText variant="cardTitle">{event.title}</AppText><ParticipantAttribution userId={event.creator_id} />{event.description ? <AppText variant="bodySmall" tone="secondary">{event.description}</AppText> : null}<View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}><TagChip participantColor={event.assign_to_both ? 'both' : colorForUser(event.assigned_user_id)} label={event.assign_to_both ? 'BOTH' : (event.assigned_user_name ?? 'ASSIGNED').toUpperCase()} />{event.location ? <TagChip subtle label={event.location.toUpperCase()} /> : null}{event.recurrence !== 'none' ? <TagChip subtle label={`↻ ${event.recurrence.toUpperCase()}`} /> : null}{(event.tags ?? []).map((tag) => <TagChip key={tag.id} subtle icon={tag.icon} iconDrawing={tag.icon_drawing} label={tag.name.toUpperCase()} />)}</View></Pressable><View style={{ gap: 8, alignItems: 'flex-end' }}><AppButton compact variant="ghost" label="Edit" onPress={() => beginEdit(event)} /><Pressable accessibilityRole="button" onPress={() => setDeleteTarget(event)}><AppText tone="muted">•••</AppText></Pressable></View></View></Card>; };
  ...
  return <AppScreen>
    ...
    <CollapsibleComposer title={editingId ? 'Edit event' : 'Calendar'} subtitle={editingId ? 'Update this event.' : 'Month, week and agenda views for everything you plan together.'} open={composerOpen} actionLabel="New event" closeLabel={editingId ? 'Cancel edit' : 'Close'} tone="accent" style={{ marginBottom: theme.spacing.lg }} onToggle={() => composerOpen ? resetForm() : setComposerOpen(true)}>
      ...
      <Pressable accessibilityRole="button" onPress={() => setAdvancedOpen((value) => !value)} style={{ flexDirection: 'row', justifyContent: 'space-between' }}><AppText variant="bodySmall" tone="accent">{advancedOpen ? 'Hide options' : 'More options'}</AppText><AppText tone="muted">{advancedOpen ? '⌃' : '⌄'}</AppText></Pressable>
    ...
    <View style={{ gap: theme.spacing.md }}>
      ...
      {!loading && calendarView === 'month' ? <><Card style={{ gap: theme.spacing.md }}><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}><AppButton compact variant="ghost" label="‹" onPress={() => shiftMonth(-1)} /><AppText variant="section">{new Intl.DateTimeFormat(undefined, { month: 'long', year: 'numeric' }).format(monthAnchor)}</AppText><AppButton compact variant="ghost" label="›" onPress={() => shiftMonth(1)} /></View><View style={{ flexDirection: 'row' }}>{['S','M','T','W','T','F','S'].map((day, index) => <View key={`${day}-${index}`} style={{ flex: 1, alignItems: 'center' }}><AppText variant="caption" tone="muted">{day}</AppText></View>)}</View><View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>{gridDates.map((day) => { const key = localDateKey(day); const dayEvents = byDay.get(key) ?? []; const inMonth = day.getMonth() === monthAnchor.getMonth(); const selected = key === selectedDay; const today = key === localDateKey(new Date()); return <Pressable accessibilityRole="button" key={key} onPress={() => setSelectedDay(key)} style={{ width: '14.2857%', minHeight: 58, padding: 5, borderRadius: 12, backgroundColor: selected ? theme.colors.accentSoft : 'transparent', opacity: inMonth ? 1 : 0.35 }}><AppText variant="bodySmall" tone={today ? 'accent' : 'secondary'}>{day.getDate()}</AppText><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 3, marginTop: 6 }}>{dayEvents.slice(0, 3).map((occurrence) => <View key={occurrence.key} style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: (() => { const owner = colorForUser(occurrence.event.creator_id); return owner === 'both' ? theme.colors.textMuted : participantPalettes[owner].accent; })() }} />)}{dayEvents.length > 3 ? <AppText variant="caption" tone="muted">+{dayEvents.length - 3}</AppText> : null}</View></Pressable>; })}</View></Card><AppText variant="section">{new Intl.DateTimeFormat(undefined, { weekday: 'long', month: 'long', day: 'numeric' }).format(new Date(`${selectedDay}T12:00:00`))}</AppText>{selectedOccurrences.length ? selectedOccurrences.map(renderOccurrence) : <EmptyState icon="☾" title="Nothing planned this day" body="Pick another day or add something." actionLabel="Add an event" onAction={() => setComposerOpen(true)} />}</> : null}
      {!loading && calendarView === 'week' ? <><Card style={{ gap: theme.spacing.md }}><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}><AppButton compact variant="ghost" label="‹" onPress={() => { const d = new Date(`${selectedDay}T12:00:00`); d.setDate(d.getDate() - 7); setSelectedDay(localDateKey(d)); }} /><AppText variant="section">Week of {new Intl.DateTimeFormat(undefined, { month: 'short', day: 'numeric' }).format(weekStart)}</AppText><AppButton compact variant="ghost" label="›" onPress={() => { const d = new Date(`${selectedDay}T12:00:00`); d.setDate(d.getDate() + 7); setSelectedDay(localDateKey(d)); }} /></View><View style={{ flexDirection: 'row', gap: 5 }}>{weekDates.map((day) => { const key = localDateKey(day); const count = weekOccurrences.filter((item) => localDateKey(item.start) === key).length; const active = key === selectedDay; return <Pressable accessibilityRole="button" key={key} onPress={() => setSelectedDay(key)} style={{ flex: 1, paddingVertical: 9, borderRadius: 12, alignItems: 'center', backgroundColor: active ? theme.colors.accentSoft : theme.colors.elevatedBackground }}><AppText variant="caption" tone={active ? 'accent' : 'muted'}>{new Intl.DateTimeFormat(undefined, { weekday: 'narrow' }).format(day)}</AppText><AppText variant="cardTitle">{day.getDate()}</AppText>{count ? <AppText variant="caption" tone="accent">{count}</AppText> : null}</Pressable>; })}</View></Card><AppText variant="section">{new Intl.DateTimeFormat(undefined, { weekday: 'long', month: 'long', day: 'numeric' }).format(new Date(`${selectedDay}T12:00:00`))}</AppText>{weekOccurrences.filter((item) => localDateKey(item.start) === selectedDay).length ? weekOccurrences.filter((item) => localDateKey(item.start) === selectedDay).map(renderOccurrence) : <EmptyState icon="☾" title="Nothing planned this day" body="Choose another day in the week or add an event." actionLabel="Add an event" onAction={() => setComposerOpen(true)} />}</> : null}
      {!loading && calendarView === 'agenda' ? <><AppText variant="section">Next 12 months</AppText>{agenda.length === 0 ? <EmptyState icon="▦" title="Your calendar is clear" body="Add your first shared event." actionLabel="Add an event" onAction={() => setComposerOpen(true)} /> : agenda.map(renderOccurrence)}</> : null}
    </View>
