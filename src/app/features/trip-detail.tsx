...
export default function TripDetailScreen() {
  ...
  return <AppScreen>
    ...
    <View style={{ gap: theme.spacing.md }}>{links.length === 0 ? <EmptyState icon="✈" title="Your trip plan is ready to build" body="Create a packing list, countdown, calendar entry, or link an existing goal." /> : links.map((item) => <Card key={`${item.entity_type}:${item.entity_id}`} participantColor="both" style={{ gap: theme.spacing.sm }}><Pressable accessibilityRole="button" onPress={() => router.push(linkHref(item) as never)} style={{ gap: 5 }}><AppText variant="caption" tone="secondary">{item.entity_type.toUpperCase()}</AppText><AppText variant="cardTitle">{item.title}</AppText>{item.subtitle ? <AppText variant="bodySmall" tone="muted">{item.subtitle}</AppText> : null}</Pressable><View style={{ flexDirection: 'row', gap: 8 }}><AppButton compact variant="ghost" label="Open" onPress={() => router.push(linkHref(item) as never)} /><AppButton compact variant="ghost" label="Unlink" onPress={() => unlink(item)} /></View></Card>)}</View>
  </AppScreen>;
}
