...
import { moodResponse, moodIsCurrent } from '@/utils/experience';
...
export default function MoodScreen() {
  ...
  async function save() {
    if (!mood || !need) return;
    setBusy(true);
    try { await createMood({ mood, need, visibility, context, validForHours: Number(hours) }); setMood(null); setNeed(null); setContext(''); await refresh(); }
  ...
  return (
    <AppScreen>
      <BackHeader eyebrow="Connect" title="Mood check-in" subtitle="Say how you are — and what would actually help." />
      <Card participantColor={myColor} style={{ gap: theme.spacing.lg, marginBottom: theme.spacing.xxl }}>
        ...
        <View style={{ gap: theme.spacing.sm }}><AppText variant="section">How are you feeling?</AppText><AppText variant="bodySmall" tone="secondary">A quick way to let your partner know.</AppText></View>
        ...
        <FormField label="Anything else? · optional" value={context} onChangeText={setContext} maxLength={300} placeholder="Just letting you know, or could we talk later?" />
        <AppText variant="bodySmall" tone="secondary">Show this as current for</AppText>
        <ChoiceChips value={hours} onChange={setHours} options={[{ value: '1', label: '1 hour' }, { value: '4', label: '4 hours' }, { value: '12', label: '12 hours' }, { value: '24', label: '24 hours' }]} />
        ...
        <AppButton label={busy ? 'Saving…' : visibility === 'private' ? 'Save privately' : 'Share check-in'} disabled={busy || !mood || !need} onPress={save} />
      </Card>
      ...
      <View style={{ gap: theme.spacing.md }}>
        ...
        {partner ? <FadeSlideIn><Card participantColor={partnerColor} style={{ gap: theme.spacing.md }}>
          ...
          {partner.context ? <AppText tone="secondary">{partner.context}</AppText> : null}
          {partner.need !== 'nothing' && moodIsCurrent(partner) ? <AppButton compact variant={acknowledged ? 'secondary' : 'primary'} label={acknowledged ? 'Support sent' : ackBusy ? 'Sending…' : moodResponse(partner.need)} disabled={ackBusy || acknowledged} onPress={acknowledge} /> : <AppText variant="bodySmall" tone="muted">{moodIsCurrent(partner) ? 'No response needed — they just wanted you to know.' : 'An earlier check-in. They may be feeling differently now.'}</AppText>}
