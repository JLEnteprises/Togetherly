        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 9 }}>
          {tags.map((tag) => (
            <Pressable key={tag.id} accessibilityRole="button" accessibilityHint="Tap to edit. Long press to delete this tag." onLongPress={() => remove(tag.id)} onPress={() => edit(tag)}
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>{tag.icon_drawing?.strokes?.length ? <DrawnIcon strokes={tag.icon_drawing.strokes} size={17} /> : tag.icon ? <AppText>{tag.icon}</AppText> : null}<AppText>{tag.name}</AppText></View><Pressable accessibilityRole="button" onPress={() => remove(tag.id)} hitSlop={8}><AppText tone="muted">×</AppText></Pressable>
            </Pressable>
          ))}
        </View>
      </View>
