  ...
  return (
    <AppScreen>
      ...
      <Card participantColor={colorForUser(list.creator_id)} style={{ gap: theme.spacing.sm, marginBottom: theme.spacing.lg }}>
        ...
        <Pressable accessibilityRole="button" onPress={() => setSettingsOpen((value) => !value)} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingTop: 4 }}><AppText variant="bodySmall" tone="accent">List settings</AppText><AppIcon name={settingsOpen ? 'chevronUp' : 'chevronDown'} size={15} color={theme.colors.textMuted} /></Pressable>
      ...
      <CollapsibleComposer title={editingItemId ? 'Edit item' : 'Items'} subtitle={`${openItems.length} still to do`} open={composerOpen} actionLabel="Add item" closeLabel={editingItemId ? 'Cancel edit' : 'Close'} tone="accent" style={{ marginBottom: theme.spacing.xxl }} onToggle={() => composerOpen ? resetItemEditor() : setComposerOpen(true)}>
        ...
        <Pressable accessibilityRole="button" onPress={() => setMoreOpen((value) => !value)} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 3 }}><AppText variant="bodySmall" tone="accent">{moreOpen ? 'Hide options' : 'More options'}</AppText><AppIcon name={moreOpen ? 'chevronUp' : 'chevronDown'} size={15} color={theme.colors.textMuted} /></Pressable>
