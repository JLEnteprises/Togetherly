...
export default function CountdownsScreen() {
  ...
  return <AppScreen>
    ...
    <View style={{ gap: theme.spacing.md }}>
      ...
      {!loading && countdowns.length === 0 ? <Card tone="secondary"><AppText tone="secondary">Create your first visit or milestone countdown.</AppText></Card> : null}
      {countdowns.map((countdown) => { const time = remaining(countdown.target_at); const passed = time.days < 0; const percent = progress(countdown.start_at, countdown.target_at); const creatorColor = colorForUser(countdown.creator_id); const palette = creatorColor === 'both' ? null : participantPalettes[creatorColor]; return <Card key={countdown.id} participantColor={creatorColor} style={{ gap: theme.spacing.md, borderColor: editingId === countdown.id ? theme.colors.accent : theme.colors.border }}><Pressable accessibilityRole="button" onPress={() => beginEdit(countdown)} style={{ gap: theme.spacing.md }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}><View style={{ flex: 1, gap: 5 }}><TagChip subtle label={countdown.type.toUpperCase()} /><AppText variant="section" style={{ marginTop: 3 }}>{countdown.title}</AppText><ParticipantAttribution userId={countdown.creator_id} /><AppText variant="bodySmall" tone="secondary">{formatTarget(countdown.target_at)}</AppText></View><AppText tone="muted">›</AppText></View><View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: theme.spacing.sm }}><AppText variant="numeric" style={{ color: passed ? theme.colors.textMuted : (palette?.accent ?? theme.colors.textSecondary) }}>{passed ? '0' : Math.max(0, time.days)}</AppText><AppText variant="cardTitle" style={{ paddingBottom: 6 }}>{passed ? 'arrived' : 'days'}</AppText></View>{!passed ? <AppText variant="caption" tone="muted">{time.weeks ? `${time.weeks} weeks · ` : ''}{time.hours.toLocaleString()} hours remaining</AppText> : null}{percent != null ? <><ProgressBar value={percent} /><AppText variant="caption" tone="muted">{Math.round(percent)}% of the wait complete</AppText></> : null}</Pressable><View style={{ flexDirection: 'row', gap: theme.spacing.sm }}><AppButton compact variant="ghost" label="Edit" onPress={() => beginEdit(countdown)} /><AppButton compact variant="danger" label="Delete" onPress={() => setDeleteTarget(countdown)} /></View></Card>; })}
    </View>
