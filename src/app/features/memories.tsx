...
export default function MemoriesScreen() {
  ...
  const [composerOpen, setComposerOpen] = useState(false); const [filter, setFilter] = useState<Filter>('all'); const [busy, setBusy] = useState(false); const [editingId, setEditingId] = useState<string | null>(null); const [detailTarget, setDetailTarget] = useState<CoupleMemory | null>(null); const [detailPhotoIndex, setDetailPhotoIndex] = useState(0); const [detailOpenPhoto, setDetailOpenPhoto] = useState(false); const [deleteTarget, setDeleteTarget] = useState<CoupleMemory | null>(null); const [loading, setLoading] = useState(true);
    ...
    <View style={{ gap: theme.spacing.md }}>
      ...
      {visible.map((memory) => <Card key={memory.id} participantColor="both" style={{ gap: theme.spacing.md, overflow: 'hidden', borderColor: editingId === memory.id ? theme.colors.accent : theme.colors.border }}>
        ...
        <View style={{ flexDirection: 'row', gap: theme.spacing.md, alignItems: 'flex-start' }}>
          <Pressable accessibilityRole="button" accessibilityLabel={`Open memory ${memory.title}`} onPress={() => { setDetailTarget(memory); setDetailPhotoIndex(0); setDetailOpenPhoto(false); }} style={({ pressed }) => ({ flex: 1, gap: 5, opacity: pressed ? 0.78 : 1 })}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}><AppText variant="section">{memory.emoji}</AppText><AppText variant="section" style={{ flex: 1 }}>{memory.title}</AppText></View><ParticipantAttribution userId={memory.creator_id} /><AppText variant="caption" tone="secondary">{memory.memory_date}{memory.location ? ` · ${memory.location}` : ''}</AppText>{memory.description ? <AppText tone="secondary" numberOfLines={3}>{memory.description}</AppText> : <AppText variant="bodySmall" tone="muted">Tap to open this memory.</AppText>}</Pressable>
