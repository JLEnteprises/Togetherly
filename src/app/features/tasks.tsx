...
export default function TasksScreen() {
  ...
  async function setStatus(task: CoupleTask, status: CoupleTask['status']) {
    ...
    try { await updateTask(task.id, { status }); await refresh(); }
    ...
    <AppScreen>
      ...
      >
        ...
        <Pressable accessibilityRole="button" onPress={() => setAdvancedOpen((value) => !value)} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 4 }}>
          <AppText variant="bodySmall" tone="accent">{advancedOpen ? 'Hide options' : 'More options'}</AppText><AppText tone="muted">{advancedOpen ? '⌃' : '⌄'}</AppText>
        </Pressable>
      ...
      <View style={{ gap: theme.spacing.md }}>
        ...
        {!loading && visibleTasks.length === 0 ? <EmptyState icon="✓" title={tasks.length ? "Nothing matches this view" : "Nothing to do yet"} body={tasks.length ? (statusFilter === 'now' ? "Nothing needs attention right now." : "Try a different status or assignment filter.") : "Add your first shared task when there’s something you want to remember together."} actionLabel={tasks.length ? undefined : "Add a task"} onAction={tasks.length ? undefined : () => setComposerOpen(true)} /> : null}
          ...
          return (
            <Card key={task.id} participantColor={creatorColor} style={{ gap: theme.spacing.md, opacity: done ? 0.68 : 1, borderColor: editingId === task.id ? theme.colors.accent : theme.colors.border }}>
              <View style={{ flexDirection: 'row', gap: theme.spacing.md, alignItems: 'flex-start' }}>
                <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: done }} onPress={() => setStatus(task, done ? 'not_started' : 'completed')} style={{ width: 28, height: 28, borderRadius: 14, borderWidth: 1, borderColor: creatorPalette?.accent ?? theme.colors.textMuted, backgroundColor: done ? (creatorPalette?.accentSoft ?? theme.colors.elevatedBackground) : 'transparent', alignItems: 'center', justifyContent: 'center', marginTop: 2 }}><AppText style={{ color: creatorPalette?.accent ?? theme.colors.textMuted }}>{done ? '✓' : ''}</AppText></Pressable>
                <Pressable accessibilityRole="button" onPress={() => beginEdit(task)} style={{ flex: 1, gap: 6 }}>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}><TagChip participantColor={assignmentColor} label={task.assign_to_both ? 'BOTH' : (task.assignee_name ?? 'ASSIGNED').toUpperCase()} /><TagChip subtle label={dueLabel(task).toUpperCase()} />{task.start_date ? <TagChip subtle label={`START ${shortDate(task.start_date).toUpperCase()}`} /> : null}{task.estimated_minutes ? <TagChip subtle label={`~${durationLabel(task.estimated_minutes).toUpperCase()}`} /> : null}{task.recurrence !== 'none' ? <TagChip subtle label={`↻ ${recurrenceLabel(task.recurrence).toUpperCase()}`} /> : null}{task.priority !== 'normal' ? <TagChip subtle label={`${task.priority.toUpperCase()} PRIORITY`} /> : null}{(task.tags ?? []).map((tag) => <TagChip key={tag.id} subtle icon={tag.icon} iconDrawing={tag.icon_drawing} label={tag.name.toUpperCase()} />)}</View>
                  {(task.subtasks ?? []).length ? <View style={{ gap: 5, marginTop: 2 }}>{(task.subtasks ?? []).map((step) => <Pressable accessibilityRole="button" key={step.id} onPress={() => toggleStep(step)} style={{ flexDirection: 'row', gap: 7, alignItems: 'center' }}><AppText tone={step.completed ? 'muted' : 'secondary'}>{step.completed ? '☑' : '☐'}</AppText><View style={{ flex: 1 }}><AppText variant="bodySmall" tone={step.completed ? 'muted' : 'secondary'} style={step.completed ? { textDecorationLine: 'line-through' } : undefined}>{step.title}</AppText>{step.due_date || step.estimated_minutes ? <AppText variant="caption" tone="muted">{step.due_date ? `Due ${shortDate(step.due_date)}` : ''}{step.due_date && step.estimated_minutes ? ' · ' : ''}{step.estimated_minutes ? `~${durationLabel(step.estimated_minutes)}` : ''}</AppText> : null}</View></Pressable>)}<AppText variant="caption" tone="muted">{(task.subtasks ?? []).filter((step) => step.completed).length}/{(task.subtasks ?? []).length} steps complete</AppText></View> : null}
                  {!done ? <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 2 }}>{task.status !== 'in_progress' ? <AppButton compact variant="secondary" label="Start" onPress={() => setStatus(task, 'in_progress')} /> : null}<AppButton compact variant="ghost" label="Skip" onPress={() => setStatus(task, 'skipped')} /><AppButton compact variant="ghost" label="Edit" onPress={() => beginEdit(task)} /></View> : <AppButton compact variant="ghost" label="Edit" onPress={() => beginEdit(task)} />}
                </Pressable>
                <Pressable accessibilityRole="button" onPress={() => setDeleteTarget(task)} accessibilityLabel={`Delete ${task.title}`} hitSlop={8}><AppText tone="muted">•••</AppText></Pressable>
              </View>
            </Card>
          );
        })}
