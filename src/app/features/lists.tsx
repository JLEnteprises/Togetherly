  ...
  return (
    <AppScreen>
      ...
      <View style={{ gap: theme.spacing.md }}>
        ...
        {!loading && lists.length === 0 ? <EmptyState icon="☷" title="Start your first shared list" body="Create a list for groceries, packing, ideas, or anything you want to keep in one place together." actionLabel="Create a list" onAction={() => setComposerOpen(true)} /> : null}
        {lists.map((list) => (
            ...
            {({ pressed }) => (
              <Card participantColor={colorForUser(list.creator_id)} style={{ gap: theme.spacing.sm, opacity: pressed ? 0.75 : 1 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: theme.spacing.md }}>
                  ...
                  <AppText variant="section" tone="muted">›</AppText>
                </View>
              </Card>
            )}
          </Pressable>
