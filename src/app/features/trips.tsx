...
export default function TripsScreen() {
  ...
  return <AppScreen>
    ...
    <View style={{ gap: theme.spacing.md }}>{loading ? <AppText tone="muted">Loading trips…</AppText> : null}{!loading && trips.length === 0 ? <EmptyState icon="✈" title="No trips planned yet" body="Create a visit, weekend away, or future holiday." actionLabel="Plan a trip" onAction={() => setComposerOpen(true)} /> : null}{trips.map((trip) => <Card key={trip.id} participantColor={colorForUser(trip.creator_id)} style={{ gap: theme.spacing.sm, borderColor: editingId === trip.id ? theme.colors.accent : theme.colors.border }}><Pressable accessibilityRole="button" onPress={() => router.push(`/features/trip-detail?id=${encodeURIComponent(trip.id)}` as never)} style={{ gap: 5 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}><View style={{ flex: 1, gap: 5 }}><AppText variant="cardTitle">{trip.title}</AppText><ParticipantAttribution userId={trip.creator_id} />{trip.destination ? <AppText tone="secondary">{trip.destination}</AppText> : null}{trip.notes ? <AppText variant="bodySmall" tone="secondary" numberOfLines={3}>{trip.notes}</AppText> : null}</View><AppText tone="muted">›</AppText></View><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>{trip.start_date ? <TagChip subtle label={trip.end_date ? `${trip.start_date} → ${trip.end_date}` : trip.start_date} /> : null}{trip.link_count ? <TagChip subtle label={`${trip.link_count} PLANNING ITEMS`} /> : null}{(trip.tags ?? []).map((tag) => <TagChip key={tag.id} subtle icon={tag.icon} iconDrawing={tag.icon_drawing} label={tag.name.toUpperCase()} />)}</View></Pressable><View style={{ flexDirection: 'row', gap: theme.spacing.sm }}><AppButton compact variant="secondary" label="Plan trip" onPress={() => router.push(`/features/trip-detail?id=${encodeURIComponent(trip.id)}` as never)} /><AppButton compact variant="ghost" label="Edit" onPress={() => beginEdit(trip)} /><AppButton compact variant="danger" label="Delete" onPress={() => setDeleteTarget(trip)} /></View></Card>)}</View>
