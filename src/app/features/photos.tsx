        ...
        {pickerOpen ? (
          <Card tone="secondary" style={{ gap: theme.spacing.sm, marginBottom: theme.spacing.lg }}>
            ...
            {memories.length === 0 ? <AppText variant="bodySmall" tone="muted">No memories yet.</AppText> : memories.map((memory) => {
              const inside = albumMemories.some((item) => item.id === memory.id);
              return <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: inside }} key={memory.id} onPress={() => toggleMemory(memory)} style={{ minHeight: 44, flexDirection: 'row', gap: 10, alignItems: 'center', paddingVertical: 6 }}><AppText tone={inside ? 'accent' : 'muted'}>{inside ? '☑' : '☐'}</AppText><AppText style={{ flex: 1 }}>{memory.emoji} {memory.title}</AppText><AppText variant="caption" tone="muted">{memory.memory_date}</AppText></Pressable>;
            })}
          </Card>
        ) : null}

        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.md }}>
          {albumMemories.length === 0 ? <View style={{ width: '100%' }}><EmptyState icon="▦" title="Nothing here yet" body="Add memories to this album." /></View> : null}
    ...
    <AppScreen>
      ...
      {view === 'all' ? (
        <>
          ...
          {!loading && photos.length === 0 ? <EmptyState icon="▣" title="No photos yet" body="Add a photo to a memory and it’ll show up here." actionLabel="Add a memory" onAction={() => router.push('/features/memories' as never)} /> : null}
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.md }}>
        ...
        <>
          ...
          {!loading && albums.length === 0 ? <EmptyState icon="▦" title="No albums yet" body="Group your favourite memories into an album." actionLabel="Create album" onAction={() => setComposerOpen(true)} /> : null}
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.md }}>
            {albums.map((album) => (
              <Card key={album.id} participantColor={colorForUser(album.creator_id)} style={{ width: '47%', padding: 0, overflow: 'hidden' }}>
                <Pressable accessibilityRole="button" onPress={() => openAlbum(album)} style={({ pressed }) => ({ opacity: pressed ? 0.76 : 1 })}>
                  {album.cover_url ? <Image source={{ uri: album.cover_url }} style={{ width: '100%', aspectRatio: 1.15, backgroundColor: theme.colors.elevatedBackground }} resizeMode="cover" /> : <View style={{ aspectRatio: 1.15, backgroundColor: theme.colors.elevatedBackground, alignItems: 'center', justifyContent: 'center' }}><AppText variant="hero" tone="accent">▦</AppText></View>}
                ...
                <View style={{ paddingHorizontal: theme.spacing.md, paddingBottom: theme.spacing.md }}><AppButton compact variant="danger" label="Delete" onPress={() => setDeleteTarget(album)} /></View>
              </Card>
            ))}
          </View>
        </>
