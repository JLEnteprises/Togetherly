...
export default function MemoryJarScreen() {
  ...
  return (
    <AppScreen>
      ...
      {!loading && !memory ? <EmptyState icon="◉" title="The jar is empty" body="Save a memory first, then come back and pull one." actionLabel="Add a memory" onAction={() => router.push('/features/memories' as never)} /> : null}
      {memory ? (
        <Card participantColor={colorForUser(memory.creator_id)} style={{ gap: theme.spacing.lg, padding: theme.spacing.xl, overflow: 'hidden' }}>
          ...
          <AppButton label="✦ Pull another memory" onPress={pull} />
        </Card>
      ) : null}
    </AppScreen>
  );
