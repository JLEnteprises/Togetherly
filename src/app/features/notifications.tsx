  ...
  return (
    <AppScreen>
      ...
      <View style={{ gap: theme.spacing.md, marginBottom: theme.spacing.xxl }}>
        ...
        {!loading && notifications.length === 0 ? <EmptyState icon="◌" title="Nothing new" body={partnerProfile ? `Updates from ${partnerProfile.display_name} and reminders you’ve chosen will appear here.` : 'Shared updates and reminders you’ve chosen will appear here.'} /> : null}
            ...
            {({ pressed }) => (
              <Card participantColor={item.actor_user_id ? colorForUser(item.actor_user_id) : 'both'} tone={item.read_at ? 'default' : 'secondary'} style={{ gap: 7, opacity: pressed ? 0.72 : 1 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}>
                  ...
                  <Pressable accessibilityRole="button" onPress={(event) => { event.stopPropagation(); remove(item.id).catch(() => undefined); }} hitSlop={8}><AppText tone="muted">×</AppText></Pressable>
                </View>
              </Card>
            )}
          </Pressable>
