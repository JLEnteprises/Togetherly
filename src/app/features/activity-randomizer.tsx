...
export default function ActivityRandomizerScreen() {
  ...
  return (
    <AppScreen>
      ...
      {hasTried && !busy && !pick ? <EmptyState icon="✦" title="Nothing matches" body="Nothing matches those filters. Try loosening one or add another idea." /> : null}
      {pick ? (
        <Card tone="accent" participantColor={colorForUser(pick.creator_id)} style={{ gap: theme.spacing.lg, padding: theme.spacing.xl }}>
          ...
          <View style={{ gap: theme.spacing.sm }}>
            <AppButton label="♥ Let’s do it" onPress={planPick} />
            <View style={{ flexDirection: 'row', gap: theme.spacing.sm }}><View style={{ flex: 1 }}><AppButton label="✦ Reroll" variant="secondary" onPress={roll} /></View><View style={{ flex: 1 }}><AppButton label="Not tonight" variant="ghost" onPress={notTonight} /></View></View>
