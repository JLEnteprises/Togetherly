...
export default function ActivitiesScreen() {
  ...
  const refresh = useCallback(async () => { try { const [nextActivities, nextTags] = await Promise.all([getActivities(), getTags()]); setActivities(nextActivities); setTags(nextTags); } catch (error) { Alert.alert('Couldn’t load activities', messageFrom(error)); } finally { setLoading(false); } }, []);
  ...
  async function setStatus(activity: CoupleActivity, status: ActivityStatus) { try { await updateActivity(activity.id, { status }); await refresh(); } catch (error) { Alert.alert('Couldn’t update activity', messageFrom(error)); } }
  async function removeConfirmed() { const target = deleteTarget; setDeleteTarget(null); if (!target) return; try { await deleteActivity(target.id); setActivities((current) => current.filter((activity) => activity.id !== target.id)); if (editingId === target.id) resetForm(); } catch (error) { Alert.alert('Couldn’t delete activity', messageFrom(error)); } }
  ...
  return <AppScreen>
    ...
    <CollapsibleComposer title={editingId ? 'Edit idea' : 'Add a date idea'} subtitle={editingId ? 'Change the details you want to keep.' : 'Start simple. Extra details are optional.'} open={composerOpen} actionLabel="New idea" closeLabel={editingId ? 'Cancel edit' : 'Close'} tone="accent" style={{ marginBottom: theme.spacing.lg }} onToggle={() => composerOpen ? resetForm() : setComposerOpen(true)}>
      ...
      {advancedOpen ? <View style={{ gap: theme.spacing.lg }}>
        ...
        <View style={{ gap: theme.spacing.sm }}><AppText variant="caption" tone="secondary">ENVIRONMENT</AppText><ChoiceChips value={environment} onChange={setEnvironment} options={[{ value: 'either', label: 'Either' }, { value: 'indoor', label: 'Indoor' }, { value: 'outdoor', label: 'Outdoor' }]} /></View>
        ...
        <View style={{ flexDirection: 'row', gap: theme.spacing.sm }}><View style={{ flex: 1 }}><FormField label="DURATION · MIN" value={duration} onChangeText={setDuration} keyboardType="number-pad" placeholder="120" /></View><View style={{ flex: 1 }}><FormField label="LOCATION · OPTIONAL" value={location} onChangeText={setLocation} placeholder="Lake, Discord…" /></View></View>
        <FormField label="RATING · OPTIONAL 1–5" value={rating} onChangeText={setRating} keyboardType="number-pad" placeholder="5" />
      ...
      <AppButton label={busy ? 'Saving…' : editingId ? 'Save changes' : 'Save idea'} disabled={busy || !title.trim()} onPress={save} />
    </CollapsibleComposer>
    ...
    <View style={{ gap: theme.spacing.md }}>
      ...
      {visible.map((activity) => {
        ...
        const statusText = myInterest && partnerInterest ? 'Both of you want this' : myInterest ? 'You’re interested' : partnerInterest ? `${partnerProfile!.display_name} is interested` : 'No interest votes yet';
        return <Card key={activity.id} participantColor={colorForUser(activity.creator_id)} style={{ gap: theme.spacing.md, opacity: activity.status === 'skip' ? 0.56 : 1 }}>
          ...
          <View style={{ flexDirection: 'row', gap: theme.spacing.sm }}>
            <View style={{ flex: 1 }}><AppButton compact variant={myInterest ? 'secondary' : 'ghost'} label={myInterest ? 'Interested ✓' : 'Interested'} onPress={() => toggleInterest(activity)} /></View>
    ...
    <ConfirmDialog visible={!!deleteTarget} title="Delete activity?" body={deleteTarget ? `Delete “${deleteTarget.title}”?` : ''} onCancel={() => setDeleteTarget(null)} onConfirm={() => removeConfirmed().catch(() => undefined)} />
  </AppScreen>;
}
