...
const noteTools = [
  { icon: '✎', title: 'Shared scratchpad', subtitle: 'One quick note both of you can update', href: '/features/scratchpad' },
] as const;
...
export default function NotesScreen() {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(null); const [title, setTitle] = useState(''); const [body, setBody] = useState(''); const [visibility, setVisibility] = useState<'shared' | 'private'>('shared'); const [pinned, setPinned] = useState(false);
  ...
  function resetEditor(close = true) { setSelectedId(null); setTitle(''); setBody(''); setVisibility('shared'); setPinned(false); setSelectedTagIds([]); if (close) setComposerOpen(false); }
  function select(note: CoupleNote) { setSelectedId(note.id); setTitle(note.title); setBody(note.body); setVisibility(note.visibility); setPinned(note.pinned); setSelectedTagIds((note.tags ?? []).map((tag) => tag.id)); setComposerOpen(true); }
  ...
  async function save() { if (!title.trim()) return; setBusy(true); try { if (selectedId) await updateNote(selectedId, { title: title.trim(), body, visibility, pinned, tagIds: selectedTagIds }); else await createNote({ title: title.trim(), body, visibility, pinned, tagIds: selectedTagIds }); resetEditor(); await refresh(); } catch (error) { Alert.alert('Couldn’t save note', messageFrom(error)); } finally { setBusy(false); } }
