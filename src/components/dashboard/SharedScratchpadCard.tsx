    ...
    <Card participantColor={compact ? 'both' : item ? ownerColor : 'both'} style={{ gap: theme.spacing.md }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: theme.spacing.md }}>
        <View style={{ flex: 1 }}>
          ...
          <AppText variant={compact ? 'cardTitle' : 'section'}>{compact ? 'Leave something for each other' : 'Shared scratchpad'}</AppText>
      ...
      {compact && !expanded ? (
        <>
          ...
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            <AppButton compact variant="ghost" label="✎ Text" onPress={() => begin('text')} />
            <AppButton compact variant="ghost" label="⌁ Draw" onPress={() => begin('draw')} />
            <AppButton compact variant="ghost" label="Open" onPress={() => router.push('/features/scratchpad' as never)} />
          </View>
        </>
      ) : (
        <>
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            <AppButton compact variant={mode === 'text' ? 'secondary' : 'ghost'} label="✎ Text" onPress={() => setMode('text')} />
            <AppButton compact variant={mode === 'draw' ? 'secondary' : 'ghost'} label="⌁ Draw" onPress={() => setMode('draw')} />
          </View>
