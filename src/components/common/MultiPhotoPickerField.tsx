...
export function MultiPhotoPickerField({ label, values, onChange, max = 8 }: {
  ...
  return <View style={{ gap: theme.spacing.sm }}>
    ...
    {values.length ? <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.sm }}>{values.map((value, index) => <View key={`${value.slice(0, 30)}-${index}`} style={{ position: 'relative' }}><Image source={{ uri: value }} style={{ width: 86, height: 86, borderRadius: theme.radii.md, backgroundColor: theme.colors.elevatedBackground }} resizeMode="cover" /><Pressable accessibilityRole="button" accessibilityLabel={`Remove photo ${index + 1}`} onPress={() => onChange(values.filter((_, itemIndex) => itemIndex !== index))} style={{ position: 'absolute', top: 4, right: 4, width: 26, height: 26, borderRadius: 13, backgroundColor: theme.colors.card, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: theme.colors.border }}><AppText>×</AppText></Pressable></View>)}</View> : <AppText variant="bodySmall" tone="muted">Add up to {max} photos to this memory.</AppText>}
