...
import { stopBackgroundLocationTask } from '@/services/locationBackground';
import { deactivatePushBeforeSignOut, forgetLocalPushRegistration } from '@/services/pushNotifications';
...
export async function changeAccountPassword(currentPassword: string, newPassword: string) {
  ...
  await forgetLocalPushRegistration().catch(() => undefined);
  await storeSession(null);
}
...
export async function signOut() {
  ...
  try {
    if (existing) {
      await deactivatePushBeforeSignOut().catch(() => undefined);
      await apiRequest<void>('/watch/sessions', { method: 'DELETE' }).catch(() => undefined);
      await apiRequest('/location/sharing', { method: 'PUT', body: { enabled: false } }).catch(() => undefined);
    }
