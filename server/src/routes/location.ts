...
function isFreshLocation(value: unknown) {
  if (!value) return false;
  const captured = new Date(String(value)).getTime();
  const now = Date.now();
  return Number.isFinite(captured) && captured <= now + 5 * 60_000 && now - captured <= LOCATION_STALE_MS;
}
...
export async function registerLocationRoutes(app: FastifyInstance, realtime: RealtimeHub) {
  app.get('/location', { preHandler: authenticate }, async (request, reply) => {
    try {
      ...
      return reply.send({ members: result.rows.map((row) => {
        const fresh = row.sharing_enabled === true && isFreshLocation(row.captured_at);
        return {
          ...
          latitude: fresh && row.latitude != null ? Number(row.latitude) : null,
          longitude: fresh && row.longitude != null ? Number(row.longitude) : null,
          accuracyM: fresh && row.accuracy_m != null ? Number(row.accuracy_m) : null,
          capturedAt: fresh && row.captured_at ? new Date(row.captured_at).toISOString() : null,
        };
      }) });
  ...
  app.put('/location/sharing', { preHandler: authenticate }, async (request, reply) => {
    try {
      ...
      if (enabled) {
        await pool.query(`INSERT INTO live_locations(user_id,couple_id,sharing_enabled) VALUES($1,$2,true)
          ON CONFLICT(user_id) DO UPDATE SET couple_id=excluded.couple_id,sharing_enabled=true,updated_at=now()`, [request.userId,coupleId]);
      } else {
        await pool.query(`INSERT INTO live_locations(user_id,couple_id,sharing_enabled,latitude,longitude,accuracy_m,captured_at) VALUES($1,$2,false,NULL,NULL,NULL,NULL)
          ON CONFLICT(user_id) DO UPDATE SET couple_id=excluded.couple_id,sharing_enabled=false,latitude=NULL,longitude=NULL,accuracy_m=NULL,captured_at=NULL,updated_at=now()`, [request.userId,coupleId]);
      }
