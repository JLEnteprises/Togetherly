  ...
  app.get('/auth/export', { preHandler: authenticate }, async (request, reply) => {
    try {
      ...
      const queries: Array<[string, string, unknown[]]> = [
        ...
        ['date_proposals', 'SELECT * FROM date_proposals WHERE couple_id=$1 ORDER BY start_at', [coupleId]],
        ['memory_reflections', 'SELECT r.* FROM memory_reflections r JOIN memories m ON m.id=r.memory_id WHERE m.couple_id=$1', [coupleId]],
        ['capsule_opens', 'SELECT o.* FROM capsule_opens o JOIN time_capsules c ON c.id=o.capsule_id WHERE c.couple_id=$1', [coupleId]],
        ['time_capsules', 'SELECT id,creator_id,title,opens_at,created_at,CASE WHEN opens_at <= now() THEN body ELSE NULL END AS body,CASE WHEN opens_at <= now() THEN photo_url ELSE NULL END AS photo_url FROM time_capsules WHERE couple_id=$1', [coupleId]],
